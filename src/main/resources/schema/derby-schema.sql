...
-- Historical cost snapshot added for profit traceability and anomaly analysis
CREATE TABLE sale_line (
  ...
  CONSTRAINT ck_sl_buying_price CHECK (buying_price >= 0),
  CONSTRAINT ck_sl_cost_total CHECK (cost_total >= 0),
...
-- Fiscal line snapshot keeps the original unit buying price and extended cost
...
-- Return line snapshot preserves original buying price and restored cost
CREATE TABLE sales_return_line (
  ...
  buying_price DECIMAL(19,4) NOT NULL DEFAULT 0,
  cost_total_refund DECIMAL(19,4) NOT NULL DEFAULT 0,
...
-- Additional reporting views only.
-- No CREATE TABLE changes. These views are designed to sit on top of the exact schema you provided.

CREATE VIEW v_bir_invoice_list AS
SELECT
    si.id AS sale_invoice_id,
    si.sale_id,
    s.sale_no,
    CAST(si.created_at AS DATE) AS invoice_date,
    si.created_at AS invoice_timestamp,
    si.invoice_no,
    si.serial_no,
    si.store_id,
    ...
    si.seller_registered_name,
    si.seller_trade_name,
    si.seller_tin_no,
    si.seller_branch_code,
    si.seller_business_address,
    si.seller_vat_registration_type,
    si.buyer_registered_name,
    si.buyer_tin_no,
    si.buyer_business_address,
    si.cash_sales,
    si.charge_sales,
    si.gross_sales,
    si.vatable_sales,
    ...
    si.zero_rated_sales,
    si.discount_total,
    si.withholding_tax_amount,
    si.total_amount_due,
    si.pos_vendor_name,
    si.pos_vendor_tin_no,
    si.pos_vendor_address,
    si.supplier_accreditation_no,
    si.accreditation_issued_at,
    si.accreditation_valid_until,
    si.bir_permit_to_use_no,
    si.permit_to_use_issued_at,
    si.atp_no,
    si.atp_issued_at,
    si.approved_series_from,
    si.approved_series_to,
    si.is_printed,
    si.printed_at,
    si.reprint_count,
    si.voided,
...
JOIN store st ON st.id = si.store_id
JOIN terminal t ON t.id = si.terminal_id;


CREATE VIEW v_bir_invoice_tax AS
SELECT
    si.id AS sale_invoice_id,
    si.invoice_no,
    CAST(si.created_at AS DATE) AS invoice_date,
    ...
    COALESCE(
        sil.tax_code,
        CASE
            WHEN sil.is_zero_rated = 1 THEN 'ZERO_RATED'
            WHEN sil.is_vat_exempt = 1 THEN 'VAT_EXEMPT'
            ELSE 'NO_TAX'
        END
    ) AS tax_code,
    COALESCE(sil.tax_rate, 0) AS tax_rate,
    SUM(
        CASE
            WHEN COALESCE(sil.is_vat_exempt, 0) = 0
             AND COALESCE(sil.is_zero_rated, 0) = 0
            THEN sil.line_net_amount - sil.line_vat_amount
            ELSE 0
        END
    ...
    SUM(
        CASE
            WHEN COALESCE(sil.is_vat_exempt, 0) = 0
             AND COALESCE(sil.is_zero_rated, 0) = 0
            THEN sil.line_vat_amount
            ELSE 0
        END
    ) AS vat_amount,
    SUM(CASE WHEN COALESCE(sil.is_vat_exempt, 0) = 1 THEN sil.line_net_amount ELSE 0 END) AS vat_exempt_sales,
    SUM(CASE WHEN COALESCE(sil.is_zero_rated, 0) = 1 THEN sil.line_net_amount ELSE 0 END) AS zero_rated_sales,
    SUM(COALESCE(sil.line_discount_amount, 0)) AS discount_amount,
    SUM(COALESCE(sil.line_gross_amount, 0)) AS gross_amount,
    SUM(COALESCE(sil.line_net_amount, 0)) AS line_net_amount
FROM sale_invoice si
JOIN sale_invoice_line sil ON sil.sale_invoice_id = si.id
GROUP BY
    si.id,
    si.invoice_no,
    CAST(si.created_at AS DATE),
    si.store_id,
    si.terminal_id,
    si.voided,
    COALESCE(
    ...
    COALESCE(sil.tax_rate, 0);


CREATE VIEW v_bir_tax_summary AS
SELECT
    CAST(si.created_at AS DATE) AS business_date,
    si.store_id,
    st.code AS store_code,
    st.name AS store_name,
    si.terminal_id,
    ...
    SUM(CASE WHEN si.voided = 1 THEN 1 ELSE 0 END) AS void_invoice_count,
    SUM(CASE WHEN si.voided = 0 THEN si.gross_sales ELSE 0 END) AS gross_sales,
    SUM(CASE WHEN si.voided = 0 THEN si.vatable_sales ELSE 0 END) AS vatable_sales,
    SUM(CASE WHEN si.voided = 0 THEN si.vat_amount ELSE 0 END) AS vat_amount,
    SUM(CASE WHEN si.voided = 0 THEN si.vat_exempt_sales ELSE 0 END) AS vat_exempt_sales,
    SUM(CASE WHEN si.voided = 0 THEN si.zero_rated_sales ELSE 0 END) AS zero_rated_sales,
    SUM(CASE WHEN si.voided = 0 THEN si.discount_total ELSE 0 END) AS discount_total,
    SUM(CASE WHEN si.voided = 0 THEN si.withholding_tax_amount ELSE 0 END) AS withholding_tax_amount,
    SUM(CASE WHEN si.voided = 0 THEN si.total_amount_due ELSE 0 END) AS total_amount_due
FROM sale_invoice si
JOIN store st ON st.id = si.store_id
JOIN terminal t ON t.id = si.terminal_id
GROUP BY
...
CREATE VIEW v_bir_monthly_machine_sales AS
SELECT
    YEAR(CAST(si.created_at AS DATE)) AS year_no,
    MONTH(CAST(si.created_at AS DATE)) AS month_no,
    si.store_id,
    st.code AS store_code,
    st.name AS store_name,
    si.terminal_id,
    t.code AS machine_code,
    si.seller_tin_no,
    si.seller_branch_code,
    COUNT(*) AS invoice_count,
    MIN(si.serial_no) AS first_serial_no,
    MAX(si.serial_no) AS last_serial_no,
    SUM(CASE WHEN si.voided = 0 THEN si.gross_sales ELSE 0 END) AS gross_sales,
    SUM(CASE WHEN si.voided = 0 THEN si.vatable_sales ELSE 0 END) AS vatable_sales,
    SUM(CASE WHEN si.voided = 0 THEN si.vat_amount ELSE 0 END) AS vat_amount,
    SUM(CASE WHEN si.voided = 0 THEN si.vat_exempt_sales ELSE 0 END) AS vat_exempt_sales,
    ...
    SUM(CASE WHEN si.voided = 0 THEN si.withholding_tax_amount ELSE 0 END) AS withholding_tax_amount,
    SUM(CASE WHEN si.voided = 0 THEN si.total_amount_due ELSE 0 END) AS total_amount_due,
    SUM(CASE WHEN si.is_printed = 1 THEN 1 ELSE 0 END) AS printed_invoice_count,
    SUM(CASE WHEN si.voided = 1 THEN 1 ELSE 0 END) AS void_invoice_count
FROM sale_invoice si
JOIN store st ON st.id = si.store_id
JOIN terminal t ON t.id = si.terminal_id
GROUP BY
    YEAR(CAST(si.created_at AS DATE)),
    MONTH(CAST(si.created_at AS DATE)),
    si.store_id,
    st.code,
    st.name,
    si.terminal_id,
...
CREATE VIEW v_pos_profit_report AS
SELECT
    s.id AS sale_id,
    s.sale_no,
    CAST(s.sold_at AS DATE) AS business_date,
    s.sold_at,
    s.store_id,
    st.code AS store_code,
    st.name AS store_name,
    ...
    s.status,
    si.invoice_no,
    COALESCE(si.voided, 0) AS invoice_voided,
    vt.gross_amount,
    vt.line_discount_total,
    vt.header_discount_total,
    vt.discount_total,
    vt.tax_total,
    vt.total_due AS net_sales,
    vt.cost_total AS cost_of_goods_sold,
    vt.profit_amount AS gross_profit,
    CASE
        WHEN vt.total_due = 0 THEN 0
        ELSE (vt.profit_amount / vt.total_due) * 100
    END AS profit_margin_pct
FROM sale s
JOIN v_sale_totals vt ON vt.sale_id = s.id
...
CREATE VIEW v_financial_statement AS
SELECT
    d.business_date,
    COALESCE(s.sales_gross, 0) AS gross_sales,
    COALESCE(s.sales_discount, 0) AS sales_discount,
    COALESCE(s.sales_net, 0) AS sales_net,
    COALESCE(s.output_vat, 0) AS output_vat,
    COALESCE(r.refund_total, 0) AS sales_returns,
    COALESCE(s.sales_net, 0) - COALESCE(r.refund_total, 0) AS net_sales_after_returns,
    COALESCE(s.cogs, 0) - COALESCE(r.return_cost_total, 0) AS net_cost_of_goods_sold,
    (COALESCE(s.sales_net, 0) - COALESCE(r.refund_total, 0))
      - (COALESCE(s.cogs, 0) - COALESCE(r.return_cost_total, 0)) AS gross_profit,
    COALESCE(p.purchase_total, 0) AS purchases,
    COALESCE(r.return_cost_total, 0) AS return_cost_total,
    COALESCE(adj.damage_total, 0) AS damage_total,
    COALESCE(adj.expire_total, 0) AS expire_total,
    COALESCE(adj.adjustment_total, 0) AS adjustment_total
FROM (
    SELECT business_date FROM (
        SELECT CAST(created_at AS DATE) AS business_date FROM sale_invoice
        UNION
        SELECT CAST(returned_at AS DATE) AS business_date FROM sales_return
        UNION
        SELECT CAST(received_at AS DATE) AS business_date FROM purchase_receipt
        UNION
        SELECT CAST(created_at AS DATE) AS business_date FROM inventory_txn
    ) q
) d
LEFT JOIN (
    SELECT
        CAST(si.created_at AS DATE) AS business_date,
        SUM(CASE WHEN si.voided = 0 THEN si.gross_sales ELSE 0 END) AS sales_gross,
        SUM(CASE WHEN si.voided = 0 THEN si.discount_total ELSE 0 END) AS sales_discount,
        SUM(CASE WHEN si.voided = 0 THEN si.total_amount_due ELSE 0 END) AS sales_net,
        SUM(CASE WHEN si.voided = 0 THEN si.vat_amount ELSE 0 END) AS output_vat,
        SUM(CASE WHEN s.status = 'POSTED' AND si.voided = 0 THEN COALESCE(vt.cost_total, 0) ELSE 0 END) AS cogs
    FROM sale_invoice si
    JOIN sale s ON s.id = si.sale_id
    LEFT JOIN v_sale_totals vt ON vt.sale_id = s.id
    GROUP BY CAST(si.created_at AS DATE)
) s ON s.business_date = d.business_date
LEFT JOIN (
    SELECT
        z.business_date,
        COALESCE(ref.refund_total, 0) AS refund_total,
        COALESCE(cost.return_cost_total, 0) AS return_cost_total
    FROM (
        SELECT CAST(returned_at AS DATE) AS business_date
        FROM sales_return
        GROUP BY CAST(returned_at AS DATE)
    ) z
    LEFT JOIN (
        SELECT
            CAST(sr.returned_at AS DATE) AS business_date,
            SUM(COALESCE(rr.amount, 0)) AS refund_total
        FROM sales_return sr
        LEFT JOIN sales_return_refund rr ON rr.sales_return_id = sr.id
        WHERE sr.status = 'POSTED'
        GROUP BY CAST(sr.returned_at AS DATE)
    ) ref ON ref.business_date = z.business_date
    LEFT JOIN (
        SELECT
            CAST(sr.returned_at AS DATE) AS business_date,
            SUM(COALESCE(rl.cost_total_refund, 0)) AS return_cost_total
        FROM sales_return sr
        LEFT JOIN sales_return_line rl ON rl.sales_return_id = sr.id
        WHERE sr.status = 'POSTED'
        GROUP BY CAST(sr.returned_at AS DATE)
    ) cost ON cost.business_date = z.business_date
) r ON r.business_date = d.business_date
LEFT JOIN (
    SELECT
        CAST(pr.received_at AS DATE) AS business_date,
        SUM(COALESCE(prl.total_cost, 0)) AS purchase_total
...
) p ON p.business_date = d.business_date
LEFT JOIN (
    SELECT
        CAST(it.created_at AS DATE) AS business_date,
        SUM(CASE WHEN it.txn_type = 'DAMAGE' THEN COALESCE(itl.total_cost, 0) ELSE 0 END) AS damage_total,
        SUM(CASE WHEN it.txn_type = 'EXPIRE' THEN COALESCE(itl.total_cost, 0) ELSE 0 END) AS expire_total,
        SUM(CASE WHEN it.txn_type = 'ADJUSTMENT' THEN COALESCE(itl.total_cost, 0) ELSE 0 END) AS adjustment_total
    FROM inventory_txn it
    LEFT JOIN inventory_txn_line itl ON itl.txn_id = it.id
    GROUP BY CAST(it.created_at AS DATE)
) adj ON adj.business_date = d.business_date;
